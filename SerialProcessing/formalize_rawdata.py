...
def main():
    #名前のリストを取得する
    datadir = "~/Users/sota/Labratory_Local/TactileSensor/back/csv"

    files = os.listdir(datadir)

    SAMPLE_RATE = 1000 #1秒で取るサンプル数[hz]
    ...
    for file_name in files:
        ...
        if len(splited_file_name) == 4: # filenameを/.でスプリットしたら出てくる配列の要素の個数 なぜ5だった?
            with open(datadir +  "/" + file_name) as f:
            ...
            a = data_lp[1]
            maxid = signal.argrelmax(a, order=1000) # 局所的最大値, 6000はデータ幅
            #print(maxid)
            start_flag = False
            end_flag = False
            ...
            if start_flag and end_flag and not os.path.exists(new_filename):
                index +=1
